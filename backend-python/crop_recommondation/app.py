...
def predict():
    ...
    try:
        ...
        # Convert season into numerical columns
        crop_df = pd.get_dummies(crop_df)

    #    columns match the traind model
        crop_df = crop_df.reindex(columns=crop_features, fill_value=0)

        # give probabilities for each crop
        probs = crop_model.predict_proba(crop_df)[0]

        # ranked top 3 crops based on probabilities
        top_indices = probs.argsort()[-3:][::-1]

        # match the model number with crop name
        crop_mapping = {
            0: "BG300",
            1: "AT362",
            2: "BG352"
        ...
        # get market details from the request
        ...
        for crop in crops:
            ...
            # define the crop verieties for Samba and Nadu
            ...
            # create morket model input sample
            sample = {
                "year": 2024,
